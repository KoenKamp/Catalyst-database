...
class CapTempParser(BaseParser):
    """"""
    root = temp
...
class CapCatParser(BaseParser):
    ...
    def interpret(self, result, start, end):
        ...
        # print()
        c = Compound()
        catname = first(result.xpath('//name/text()'))
        # print(catname)
...
flowunit = R(r"^((h(-|−)1|ml|s|min|cm3|cat(alyst)?|l)(-|−)?\d*)$", re.IGNORECASE)
...
class CapGHSVParser(BaseParser):
    root = flow
    # root = OneOrMore(Any())(u"test")
    def __init__(self):
        pass
    ...
    def interpret(self, result, start, end):
        ...
        if flowvalue and flowunit:
            context["flow"] = flowvalue[0]
            context["flowunit"] = flowunit
        # print(context)
        if context:
            c.conv.append(Conv(**context))
        yield c
...
name = (R("^H2$") | R("^H2O$") | R("^CO$")| R("^CO2$"))(u"compname")
...
class CapCompParser(BaseParser):
    root = comp
        ...
        if test or test2:
            ...
            if abs(matchlocation - fol) > abs(matchlocation - pre):
                ...
                if re.search(r"<\/compvalue>((<COLON>:<\/COLON>)|(<SYM>\/<\/SYM>))<compvalue>", string):
                    ...
                    if len(firstvalue) > 0 and float(value[0]) != 0:
...
class CapPressureParser(BaseParser):
    """"""
    root = pres
