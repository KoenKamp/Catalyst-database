...
class RscSearchDocument(Entity):
    ...
    process_html_url = Chain(normalize, LStrip('https://pubs.rsc.org/en/content/articlepdf'), LAdd('https://pubs.rsc.org/en/content/articlehtml/'))
...
class RscSearchScraper(SearchScraper):
    ...
    def perform_search(self, query, page):
        ...
        response = self.http.get('https://pubs.rsc.org/en/results', params={'searchtext': query, 'SortBy': 'Relevance', 'PageSize': 100})
        # print("response :    \n", response.text)
        selector = Selector.from_html(response)
        # print(selector)
        sessionkey = selector.css('#SearchTerm::attr("value")').extract()[0]
        # print(selector.css('#SearchTerm::attr("value")').extract())
        # print(sessionkey)

        searchdata = {'searchterm': sessionkey, 'resultcount': "100", 'category': 'journal', 'pageno': page}
        # print(self.http.cookies.get_dict())
        response = self.http.post('https://pubs.rsc.org/en/search/journalresult', data=searchdata)
        # print(bla)
